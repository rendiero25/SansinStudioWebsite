...
const Footer = ({
  ...
  const ctaBtn = data.ctaButton || { text: "Set a discovery meet", link: "/contact" };
        ...
        <div className="relative z-10 container mx-auto px-10 md:px-12 xl:px-20 pt-20">
          ...
          <div className="relative w-full bg-[#D3B4F6] rounded-[30px] overflow-hidden flex flex-col mb-24 shadow-2xl">
            ...
            <div className="w-full h-[300px] md:h-[450px] relative">
              <img
                src={data.ctaImage?.url || "https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80"}
            ...
            {/* Bottom: Text Area */}
            <div className="w-full p-8 md:p-12 flex flex-col md:flex-row justify-between items-end gap-10">
              <h2 className="text-[28px] md:text-[42px] font-normal leading-[1.1] text-black m-0 max-w-[800px]">
                {renderStyledText(data.ctaHeading || "Lets create your _profitable plan_ through our _discovery meet_ session.")}
              </h2>
              
              <div className="flex flex-col items-end gap-4 shrink-0">
                 {data.ctaNote && (
                   <p className="text-[11px] md:text-[13px] font-medium text-black/60 m-0 italic text-right max-w-[200px]">
                      {data.ctaNote}
                   </p>
                 )}
                 <Link
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start xl:items-end gap-12 lg:gap-8 mb-32">
            ...
            <div className="flex flex-col gap-6">
              <p className="text-[14px] md:text-[15px] font-medium text-white/80 m-0 leading-tight w-[180px]">
                {data.contactTitle || "Want to discover more with us? please do"}
              </p>
              <a
                href={`mailto:${email}`}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-12 md:gap-20 xl:gap-24">
              <div className="flex flex-col gap-5">
                <Link to="/works" className="text-[20px] md:text-[24px] text-white/40 hover:text-white transition-colors no-underline">Works</Link>
                <Link to="/solutions" className="text-[20px] md:text-[24px] text-white/40 hover:text-white transition-colors no-underline">Solutions</Link>
              </div>
              <div className="flex flex-col gap-5">
                <Link to="/payments" className="text-[20px] md:text-[24px] text-white/40 hover:text-white transition-colors no-underline flex items-center gap-2">
                  Payments
                </Link>
                <Link to="/company" className="text-[20px] md:text-[24px] text-white/40 hover:text-white transition-colors no-underline">Company</Link>
              </div>
              <div className="flex flex-col gap-5">
                <Link to="/insights" className="text-[20px] md:text-[24px] text-white/40 hover:text-white transition-colors no-underline">Insights</Link>
                <Link to="/faqs" className="text-[20px] md:text-[24px] text-white/40 hover:text-white transition-colors no-underline">FAQs</Link>
              </div>
            </div>
          </div>
                ...
                {data.socialLinks && data.socialLinks.length > 0 ? (
                    ...
                    {data.socialLinks.map((s) => (
                      ...
                      >
                        {s.icon?.url ? (
                          ...
                          <span className="text-white text-[10px]">{s.platform.slice(0, 2)}</span>
                        )}
                      </a>
                    ))}
                  </div>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4">
                      {[
                        { platform: "Instagram", icon: "https://cdn-icons-png.flaticon.com/512/2111/2111463.png" },
                        { platform: "X", icon: "https://upload.wikimedia.org/wikipedia/commons/5/53/X_logo_2023_white.svg" },
                        { platform: "Threads", icon: "https://upload.wikimedia.org/wikipedia/commons/9/9d/Threads_logo.svg" },
                        { platform: "TikTok", icon: "https://cdn-icons-png.flaticon.com/512/3046/3046121.png" }
                      ].map((s) => (
                        <a key={s.platform} href="#" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-all p-2.5">
                          <img src={s.icon} alt={s.platform} className="w-full h-full object-contain invert brightness-200" />
                        </a>
                      ))}
                    </div>
                    <div className="flex items-center gap-4">
                      {[
                        { platform: "Behance", icon: "https://cdn-icons-png.flaticon.com/512/1051/1051221.png" },
                        { platform: "Dribbble", icon: "https://cdn-icons-png.flaticon.com/512/1051/1051225.png" },
                        { platform: "LinkedIn", icon: "https://cdn-icons-png.flaticon.com/512/1051/1051233.png" }
                      ].map((s) => (
                        <a key={s.platform} href="#" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-all p-2.5">
                          <img src={s.icon} alt={s.platform} className="w-full h-full object-contain invert brightness-200" />
                        </a>
