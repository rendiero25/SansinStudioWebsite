  ...
  return lines.map((line, lineIndex) => {
    ...
    const lineContent = parts.map((part, i) => {
      if (part.startsWith("_") && part.endsWith("_")) {
        ...
        return (
          <span key={i} className="italic underline underline-offset-4 decoration-[1px] text-[#5D2E8C]">
            {part.slice(1, -1)}
          </span>
        );
      }
...
const Footer = () => {
  ...
  const bgImage = data.backgroundImage?.url || "https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&q=80"; 
  const ctaHeading = data.ctaHeading || "Lets create your _profitable plan_\nthrough our _discovery meet_ session.";
  const ctaBtn = data.ctaButton || { text: "Set a discovery meet", link: "/contact" };
  const ctaNote = data.ctaNote || "*Please pay attention to our\nschedule availability";
      ...
      <div className="container relative mx-auto px-5 md:px-12 xl:px-32 2xl:px-0 max-w-[1440px] z-10">
        ...
        <div className="w-full flex flex-col rounded-3xl overflow-hidden drop-shadow-2xl mb-24 md:mb-32">
          ...
          <div className="w-full h-[250px] md:h-[400px] relative">
            <img src={data.ctaImage?.url || bgImage} alt="Discovery Session" className="w-full h-full object-cover" />
          </div>
        ...
        <div className="w-full flex flex-col lg:flex-row justify-between items-start gap-16 lg:gap-8 border-b border-white/[0.08] pb-24 md:pb-32">
          ...
          <div className="flex flex-col gap-4">
            <p className="font-primary text-[15px] md:text-[16px] text-white/80 m-0 leading-[1.3] max-w-[200px]">
              Want to discover more<br/>with us? please do
            </p>
            <a href={`mailto:${email}`} className="font-primary text-[32px] md:text-[46px] font-medium text-white/80 hover:text-white transition-colors m-0 tracking-[-0.02em]">
              {email}
            </a>
          </div>
          ...
          <div className="flex grid-cols-2 sm:grid-cols-3 gap-12 sm:gap-24 lg:gap-32">
            <div className="flex flex-col gap-5">
              <a href="#" className="font-primary text-[18px] md:text-[20px] text-white/60 hover:text-white transition-colors">Works</a>
              <a href="#" className="font-primary text-[18px] md:text-[20px] text-white/60 hover:text-white transition-colors">Services</a>
            </div>
            <div className="flex flex-col gap-5">
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center md:items-end gap-12 py-16">
          <div className="w-full md:w-auto h-auto max-w-[300px] shrink-0">
             ...
             {data.logo?.url ? (
               <img src={data.logo.url} alt="Sanxin Studio" className="h-[60px] md:h-[80px] w-auto object-contain brightness-0 invert" />
             ) : (
               <h1 className="font-primary text-[60px] md:text-[100px] font-bold text-white tracking-[-0.04em] m-0 leading-none">sanxin</h1>
             )}
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {(data.socialLinks || Array(5).fill(null)).map((item, i) => (
                <a key={item?.id || i} href={item?.url || "#"} className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/40 transition-colors">
                  {item?.icon?.url ? (
                     <img src={item.icon.url} alt={item.platform} className="w-5 h-5 object-contain" />
                  ) : (
